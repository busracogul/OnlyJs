...
charactersDiv.classList.add("row");
document.body.appendChild(charactersDiv);

function renderCharacters() {
    if (renderStatus) {
        for (const characterList of allCharacters) {
            for (const character of characterList.characters) {
                const card = `
                <div class="card card-wrapper mx-auto my-4" style="width: 18rem;">
                    <img src="${character.pic}" class="card-img-top img-wrapper" alt="...">
                    <div class="card-body">
                        <h5 class="card-title">${character.name}</h5>
                        <p class="card-text">${character.homeworld || "Bilinmiyor"}</p>
                    </div>
                </div>`;
